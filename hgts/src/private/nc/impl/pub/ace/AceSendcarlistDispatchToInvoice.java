	...
	public void process(AggSendCarListHVO aggvo) {
		...
		if(Integer.parseInt(head.getAttributeValue("vbillstatus").toString())==ApproveStatus.APPROVED){
			...
			try {
				...
				shvo = (SendnoticebillHVO) this.getDao().retrieveByPK(SendnoticebillHVO.class,HgtsPubTool.getStringNullAsTrim(pk_sendnoticebill));
				...
				for(int i=0;i<bodys.length;i++){
					...
					hvo.setAttributeValue("sendnoticebillno", shvo.getAttributeValue("vbillno"));
					...
					hvo.setAttributeValue("pk_balatype", shvo.getAttributeValue("pk_balatype"));//结算方式
					hvo.setAttributeValue("pk_busitype", shvo.getAttributeValue("pk_busitype"));//业务类型
					hvo.setAttributeValue("pk_supplier", shvo.getAttributeValue("pk_supplier"));//运输公司
					hvo.setAttributeValue("yfxycode", shvo.getAttributeValue("yfxycode"));//运输合同主键
					hvo.setAttributeValue("zxxycode", shvo.getAttributeValue("zxxycode"));//装卸合同主键
				...
				if(null !=retvos && retvos.length>0){
					for(AggInvoicesheetHVO bill:retvos){
						...
						String sql="update hgts_sendcarlist_b set invoicebillno='"+vbillno+"' , isdispatch='Y'  where pk_sendcarlist_b='"+bvos[0].getAttributeValue("csrcbid")+"'";
