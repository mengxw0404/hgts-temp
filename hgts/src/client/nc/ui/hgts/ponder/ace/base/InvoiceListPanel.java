...
public class InvoiceListPanel extends BillListPanel implements BillTableMouseListener, BillEditListener,
IBillModelRowStateChangeEventListener {
	...
	public InvoiceListPanel(BillWorkPanel parents) {
	...
	private void initlize() {
		...
		loadTemplet("40H10604", null, this.m_userid, this.m_corpid);
...
//				flag_data="2";
			...
			this.getNewTListPanel().getBodyTable().setSortEnabled(false);
	...
	public void bodyRowChange(BillEditEvent e){
		...
		try {
			...
			//司磅员及所属部门
			// 0:进 保留原始数据 ; 1:出: 根据当前操作员 设置司磅员及 所属部门
			if(getParents().getFReceOrSend() == 1){
...
//			if(getParents().getFReceOrSend() == 0){
//				getParents().setBillModel(-1);
//			}else{
//				getParents().setBillModel(0);
//				this.addBodyEditListener(getNewTListPanel());
//			}
			getParents().setBillModel(0);
			//getParents().setBillModel(-1);
	...
	protected InvoiceListPanel getNewTListPanel() {
		return getParents().getNewTListPanel();
	}
