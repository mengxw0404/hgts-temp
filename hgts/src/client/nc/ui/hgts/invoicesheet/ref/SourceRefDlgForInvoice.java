package nc.ui.hgts.invoicesheet.ref;
...
public class SourceRefDlgForInvoice extends FfPubSourceRefDlg {


	/**
	 * 
	 */
	private static final long serialVersionUID = -485760313889434711L;



	public SourceRefDlgForInvoice(Container parent, BillSourceVar bsVar) {
		super(parent, bsVar);
	...
	public String getBodyCondition() {
		...
		if(this.getBodyTableName().equals("hgts_sendnoticebill_b")){
			...
			sql = " hgts_sendnoticebill_b.shul > nvl(hgts_sendnoticebill_b.yzxnum,0) and nvl(hgts_sendnoticebill_b.rowcloseflag,'N')='N' and nvl(hgts_sendnoticebill_b.blatest ,'N')='Y'";
		}
	...
	public void loadBodyData(int arg0) {
		...
		try {
			//获得主表ID
			String id = getbillListPanel().getHeadBillModel().getValueAt(arg0, getBillSourceVar().getPkField()).toString();
			//查询子表VO数组
			SendnoticebillBVO[] bvos=(SendnoticebillBVO[]) HYPubBO_Client.queryByCondition(SendnoticebillBVO.class, getBodyCondition()+" and nvl(dr,0)=0 and pk_sendnoticebill='"+id+"'");
